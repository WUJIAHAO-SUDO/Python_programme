...
if ndims(concentration_p2d) > 1
    concentration_p2d = concentration_p2d[:, t_index]
end

if ndims(stress_p2d) > 1
    stress_p2d = stress_p2d[:, t_index]
end

if ndims(concentration_p_p2d) > 1
    concentration_p_p2d = concentration_p_p2d[:, t_index]
end

if ndims(stress_p_p2d) > 1
    stress_p_p2d = stress_p_p2d[:, t_index]
end
...
if ndims(concentration_sP2D) > 1
    concentration_sP2D = concentration_sP2D[:, t_index]
end

if ndims(stress_sP2D) > 1
    stress_sP2D = stress_sP2D[:, t_index]
end

if ndims(concentration_p_sP2D) > 1
    concentration_p_sP2D = concentration_p_sP2D[:, t_index]
end

if ndims(stress_p_sP2D) > 1
    stress_p_sP2D = stress_p_sP2D[:, t_index]
end
...
# 合并网格和数据
x_grid_combined = vcat(x_grid_n, x_grid_p)
concentration_p2d_combined = vcat(concentration_p2d, concentration_p_p2d)
concentration_sP2D_combined = vcat(concentration_sP2D, concentration_p_sP2D)
stress_p2d_combined = vcat(stress_p2d, stress_p_p2d)
stress_sP2D_combined = vcat(stress_sP2D, stress_p_sP2D)

# 创建浓度图
p2 = plot(x_grid_p, concentration_p_p2d, 
    label="Concentration (P2D)", 
    xlabel="x [m]", 
    ylabel="Concentration [mol/m³]", 
    lw=2, color=:blue,xlims=(0,1),ylims=(18300,30000))
plot!(x_grid_p, concentration_p_sP2D, 
    label="Concentration (sP2D)", 
    title="Electrode Concentration at t=1000s", 
    linestyle=:dash, color=:fuchsia)
plot!(x_grid_n, concentration_p2d, 
    label="Concentration (P2D)", 
    lw=2, color=:blue)
plot!(x_grid_n, concentration_sP2D, 
    label="Concentration (sP2D)", 
    linestyle=:dash, color=:fuchsia)

# 创建应力图
p3 = plot(x_grid_p, stress_p_p2d, 
    label="Stress (P2D)", 
    xlabel="x [m]", 
    ylabel="Stress [Pa]", 
    lw=2, color=:blue)
plot!(x_grid_p, stress_p_sP2D, 
    label="Stress (sP2D)", 
    title="Electrode Stress at t=1000s", 
...
p6 = plot(x_grid_n, concentration_error, 
    label="Concentration Error", 
    xlabel="x [m]", 
    ylabel="Error [mol/m³]", 
    title="Negative Electrode Concentration Error at t=1000s", 
    lw=2, color=:blue)

p7 = scatter(x_grid_n, stress_error, 
    label="Stress Error", 
    xlabel="x [m]", 
    ylabel="Error [Pa]", 
    title="Negative Electrode Stress Error at t=1000s", 
